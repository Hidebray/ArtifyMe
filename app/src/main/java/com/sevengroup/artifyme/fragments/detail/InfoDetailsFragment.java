...
public class InfoDetailsFragment extends Fragment {
    public static InfoDetailsFragment newInstance() { return new InfoDetailsFragment(); }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        txtTime = view.findViewById(R.id.txtInfoTime);
        txtResolution = view.findViewById(R.id.txtInfoResolution);
        txtSize = view.findViewById(R.id.txtInfoSize);
        txtPath = view.findViewById(R.id.txtInfoPath);
        ...
        if (currentProjectId != -1L) {
            ...
            viewModel.getProjectHistory(currentProjectId).observe(getViewLifecycleOwner(), versions -> {
                if (versions != null) {
                    ...
                    String imageName = file.exists() ? file.getName() : "Unknown";
                    String imageTime =  DateUtils.formatDateTime(versions.get(0).createdTime);
                    String imageSize = BitmapUtils.getImageSizeInByte(versions.get(0).imagePath) / 1000 + "KB";

                    Pair resolution = BitmapUtils.getImageResolution(imagePath);
                    String imageResolution = resolution.first + "x" + resolution.second;
